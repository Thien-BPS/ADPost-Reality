export const glyphSacrifice = {
  "power": {
    id: "power",
    effect: added => {
      if (Pelle.isDisabled("glyphsac") && !devVars.reality.forceSacrifice) return 0;
    ...
    description: amount => {
      ...
      const nextGalaxyText = amount < 7500
  ...
  "infinity": {
    id: "infinity",
    effect: added => {
      if (Pelle.isDisabled("glyphsac") && !devVars.reality.forceSacrifice) return 1;
  ...
  "replication": {
    ...
    description: amount => {
      ...
      const nextGalaxyText = amount < 15000
        ? ` (next at ${format(nextDistantGalaxy, 2, 2)})`
        : "";
  ...
  "dilation": {
    ...
    effect: added => {
      ...
      const exponent = 0.35 * Math.pow(Math.log10(capped + 1) /
        Math.log10(GlyphSacrificeHandler.maxSacrificeForEffects), 0.2);
